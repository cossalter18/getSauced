...
class UserFav extends Component {
  ...
  // }
  removeFav = (event, post) => {
    console.log('remove clicked', post.id);
    ...
    return (
      <>
        <div>
          ...
          {this.props.reduxState.favoritesReducer.map((post, index) => (
            
                  <div key={post.id}className="scroll">
                    <h2>{post.title}</h2>
                    <h6>{post.body}</h6>
                    <button className="removeBut" onClick={(event) =>this.removeFav(event, post)}>Remove</button>
                  </div>
